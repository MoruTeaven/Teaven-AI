...
- 任务支持 `store_output` 控制是否转存结果文件，默认不转存，任务完成后直接返回上游链接，以减少等待 R2 下载和上传的时间。
...
官方或内置插件可以先提供 OpenAI 兼容、Cloudflare Workers AI、硅基流动、模力方舟等实现；其他供应商，例如讯飞星辰或私有模型服务，也应通过同一插件接口接入。
...
- 模型分组：把多个同模态模型组合成一个对外别名（如 `tier:advanced`），用户调用时按权重随机挑选成员，失败自动回退；响应返回组别名但用量记录保留实际模型，可通过 `request_id` 反查实际调用链。支持一键初始化默认三组骨架。
