...
| Provider | 上游模型供应商，例如硅基流动、模力方舟。 |
| Provider Credential | 平台访问上游的密钥或凭证。 |
| Model Alias | 对外暴露的模型名，例如 `deepseek-chat`。 |
| Model Route | 模型别名到一个或多个上游模型的路由规则。 |
...
| 组成 | 说明 |
| --- | --- |
| Plugin Manifest | 插件 ID、名称、版本、能力、配置 schema、默认模型映射。 |
...
- 新增上游只需要实现插件接口、注册插件、配置凭证和模型路由。
- 模型路由只依赖插件 ID 和上游模型名，不依赖具体代码分支。
...
4. 部署后在数据库中启用插件。
5. 配置 Provider Credential。
6. 将平台模型别名路由到插件和上游真实模型。
7. 可选，执行插件 `healthCheck` 验证凭证和网络可用性。
...
- 精确匹配外部模型别名。
- 路由目标使用 `plugin_id + provider_model + credential_id`。
- 按租户或 Key 覆盖默认路由。
- 支持权重分流。
...
### 9.5 provider_credentials

| 字段 | 说明 |
| --- | --- |
| id | 凭证 ID。 |
| plugin_id | Provider Plugin ID。 |
| provider | 上游供应商展示名或兼容协议类型。 |
| name | 凭证名称。 |
| secret_ref | Cloudflare secret 或加密引用。 |
| config | 非密钥配置 JSON，例如 base_url、region、api_version。 |
| status | active、disabled。 |
...
插件、模型和路由配置建议支持热更新。新增上游时，先注册或启用 Provider Plugin，再配置凭证和模型路由。
...
1. 实现或引入 Provider Plugin。
2. 注册 Manifest 到 Provider Registry。
3. 部署 Worker 或配置 Remote Plugin endpoint。
4. 在管理侧启用插件。
5. 配置 Provider Credential。
6. 配置模型别名和 `plugin_id + provider_model + credential_id` 路由。
7. 执行健康检查和测试调用。
