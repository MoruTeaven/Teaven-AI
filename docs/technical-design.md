...
| Upstream | 已配置的上游实例，绑定插件、协议、endpoint、区域、凭证和运行状态。 |
| Provider Credential | 平台访问上游的密钥或凭证引用。 |
| Model Alias | 对外暴露的模型名，例如 `deepseek-chat`。 |
| Model Route | 模型别名到一个或多个 `upstream_id + provider_model` 的路由规则。 |
...
- 新增上游只需要实现或复用插件接口、注册插件、配置上游实例和模型路由。
- 模型路由只依赖 `upstream_id` 和上游模型名，不依赖具体代码分支。
...
4. 部署后在数据库中启用插件。
5. 按协议配置上游实例，包含 endpoint、区域、凭证引用和协议参数。
6. 将平台模型别名路由到上游实例和上游真实模型。
...
- 精确匹配外部模型别名。
- 路由目标使用 `upstream_id + provider_model`。
- 按租户或 Key 覆盖默认路由。
- 支持权重分流。
...
| provider | 上游供应商展示名。 |
| protocol | 协议或接口形态，例如 `openai-chat-completions`、`async-polling-task`、`async-webhook-task`。 |
| name | 管理后台展示名。 |
...
插件、上游实例、模型和路由配置建议支持热更新。新增上游时，先按协议注册或启用 Provider Plugin，再配置上游实例，最后配置模型路由。
...
7. 配置上游实例，绑定 `plugin_id`、协议参数、endpoint 和凭证。
8. 配置模型别名和 `upstream_id + provider_model` 路由。
9. 执行健康检查和测试调用。
