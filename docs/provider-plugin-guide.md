...
Provider Plugin 只描述一类协议或供应商适配能力，不等同于一个具体上游账号。具体协议类型、endpoint、区域、凭证和运行状态应配置成上游实例，模型再作为该上游实例下的条目添加。
...
5. 配置上游实例，写入协议类型、`plugin_id`、协议参数、endpoint、区域和凭证引用。
6. 执行上游实例的 `healthCheck`。
7. 在上游实例下添加模型条目，填写 `alias`、`provider_model` 和模型能力，不填写密钥或域名。
